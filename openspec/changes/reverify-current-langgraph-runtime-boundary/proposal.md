...
以「現在存在」的程式碼與部署，重新驗證 LangGraph Agent Server 與 Chat-Gun 自建 Runtime 模組之間的責任邊界，產出一份可追溯的 Decision Record（`docs/decisions/current-langgraph-runtime-boundary.md`），證明哪些生命週期責任歸 LangGraph Agent Server，哪些歸 Chat-Gun 的 Task／ToolExecution／Interaction／Operations 模組，並作為 X12 之後所有 Runtime 整合工作的唯一前置事實來源。
...
2. 自 X0 之後，程式碼已大幅演進。目前（本 Change 建立時）的實際狀態：
   ...
   - `docker-compose.yml` 同時執行 `langgraph-api`（LangGraph Agent Server）、`langgraph-postgres`（PostgreSQL 16）與 `langgraph-redis`（Redis 6），但未在 compose 中看到 Server 層的 explicit checkpointer 設定。
...
若在未重新驗證的情況下直接整合，風險是：自建模組可能在不知情下重複實作 LangGraph Agent Server 已內建的 Queue／Worker／Checkpointer／Store 能力，或反過來假設 Server 會處理它其實不管的邊界（例如副作用冪等、業務 Task/Step 語意、審計、成本），造成 Runtime 缺陷或責任漂移。

## 解決方案

執行一個研究型／spike 型 OpenSpec Change，輸出單一 Decision Record。工作分為兩大類：

1. **現況盤點（靜態）**：逐一確認四個 Graph 的編譯方式與 checkpointer 使用、`langgraph.json` 的 Graph 註冊與 configurable headers、`docker-compose.yml` 的 infra 組合。
2. **Agent Server 生命週期實證（動態 live spike）**：對真實 Agent Server 部署執行 create Run → stream → interrupt → restart → resume → cancel → terminal lookup 的完整循環，並驗證 Graph-level explicit checkpointer、Server-managed persistence 與無持久化三種來源的實際行為差異。
...
- `deep_researcher` 的 `MemorySaver` 是 override、complement 還是 conflict。
...
- `backend`：LangGraph Runtime 驗證主體（Graph 編譯、checkpointer、Agent Server）。
...
- 以真實 Agent Server 生命週期證明 interrupt/resume 行為與持久化邊界。
- 明確分類 `MemorySaver` 的行為，並給出「保留、後續移除、或隔離到測試」的處置建議。
...
| 無 checkpointer 的 Graph 在 Server 上的持久化行為與預期不同 | 分別記錄「有／無 checkpointer」兩類 Graph 的實測結果，不臆測 |
| `MemorySaver` 為 in-memory、跨 process 無法持久 | 明確分類其對 Server-managed persistence 的 override／complement／conflict 關係，並給出隔離或移除建議 |
| 原生能力隨 LangGraph 版本更新而變化 | Decision Record 標註驗證時的 LangGraph 版本與部署資訊 |
| live spike 需真實 infra（PG/Redis） | 使用 `docker-compose.yml` 既有服務；無法執行時在 Decision Record 標記為「未驗證」而非宣稱通過 |
