# trusted-tool-authorization Specification Delta

## Purpose

本規格定義 production Tool authorization boundary 的正式需求：把既有 `runtime-identity-permission-governance` 的 trusted identity、scope authorization、grant、decision persistence 原語，接上 Agent Server trusted-header 邊界與 `applyToolGovernance` 的 dispatch 路徑，使每個受保護 Tool action 在 production 都通過「trusted identity → scope authorization → persistent decision → durable confirmation」的強制邊界。
